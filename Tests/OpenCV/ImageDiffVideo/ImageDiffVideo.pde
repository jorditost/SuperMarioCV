...
Capture video;
OpenCV opencv;
PImage  background, current, grayDiffImage, thresholdImage, edgesImage, contoursImage;
ArrayList<Contour> contours;
Boolean backgroundInitialized;
...
void draw() {
  ...
  if (backgroundInitialized) {
    ...
    pushMatrix();
    scale(0.5);
    image(background, video.width, 0);
  ...
  fill(255);
  text("current", 10, 20);
  text("background", video.width/2 +10, 20);
...
void initBackground() {
  ...
  //  arraycopy(video.pixels, background.pixels);
